...
int global_vchar(FTYPE pointspeed[][N2M][N3M][NPR], int dir, int is, int ie, int js, int je, int ks, int ke, int idel, int jdel, int kdel, FTYPE wspeed[][NUMCS][N1M][N2M][N3M])
{
  int i,j,k;
  int reallim;
  int m;
  ...
  ZSLOOP( is-idel, ie+idel, js-jdel, je+jdel, ks-kdel, ke+kdel ) { // due to averaging of this face quantity to get centered i/j/k=0 back
    reallim=choose_limiter(i,j,k,RHO); // base stencil size on density limiter GODMARK
  ...
  ZSLOOP( is-idel, ie+idel, js-jdel, je+jdel, ks-kdel, ke+kdel ) { // due to averaging of this face quantity to get centered i/j/k=0 back
    ...
    wspeed[dir][CMAX][i][j][k]=-BIG; // assume all zones are larger than this
    // GODMARK: 1D, could do multi-D stencil even if interpolation is 1D
    // e.g. if dir=1, then expandi=0 and so FZLOOP from i=0..N inclusive.  So can grab from (relative to i) -2 .. 1 inclusive for average centered on i edge
  ...
  ZSLOOP( is-idel, ie+idel, js-jdel, je+jdel, ks-kdel, ke+kdel ) {
