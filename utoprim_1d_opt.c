...
int Utoprim_1d_opt(FTYPE U[NPR], struct of_geom *ptrgeom,  int *lpflag,  FTYPE prim[NPR])
{
...
static int Utoprim_new_body(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
  ...
  FTYPE x_1d[NEWT_DIM];
  FTYPE gcov[NDIM][NDIM], gcon[NDIM][NDIM];
  ...
  if( ltrace ) {
    ...
    dualfprintf(fail_file,"debugfail = %d \n", debugfail);
    fflush(fail_file);
  }
#endif
  ...
  *glpflag= UTOPRIMNOFAIL;
  ...
  rho0 = prim[RHO] ;
  u = prim[UU] ;
  p = pressure_rho0_u_1dopt(rho0,u) ;
  w = rho0 + u + p ;
  gammasq = 1. + utsq ;
  W_last = w*gammasq ;
  ...
  x_1d[0] = W_last;
  wglobal=w;
  ret = general_newton_raphson( x_1d, 1, USE_LINE_SEARCH, func_1d_orig, res_sq_1d_orig );
#if(!OPTIMIZED)
  if( ret ) { 
    dualfprintf(fail_file, "general_newton_raphson() failed, x_1d[0] = %21.15g \n", x_1d[0] );  fflush(fail_file);
//    dualfprintf(fail_file, "general_newton_raphson() failed, t,i,j, p[0-7], U[0-7] = %21.15g %d %d ", t, ptrgeom->i, ptrgeom->j );  
//    for( i = 0 ; i < NPR; i++ ) {
...
#if(!OPTIMIZED)
  if( ltrace ) {
    dualfprintf(fail_file,"(W,W_last,Bsq,Qtsq,QdotB,gammasq,Qdotn,gam) %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g\n",
...
//      dualfprintf(fail_file, "\n");
    ...
    *glpflag= ret*100+1;// related to UTOPRIMFAILCONVRET
    return(0);
  }
  else{
    Wtest=W/wglobal;
...
//	dualfprintf(fail_file, "\n");
  ...
  if(utsq < 0. || utsq > UTSQ_TOO_BIG) { // takes care of W_TOO_BIG
    if( debugfail>=2 ) { 
      dualfprintf(fail_file,"utsq failure:  utsq = %21.15g , W = %21.15g \n",utsq, W) ;
...
//      dualfprintf(fail_file, "\n");
  ...
  w = W / (gamma*gamma);
  p = pressure_rho0_w_1dopt(rho0,w) ;
  u = w - (rho0 + p) ;
...
#if(!OPTIMIZED)
  if( ltrace ) {
    dualfprintf(fail_file," debugfail,  final rho, uu = %d  %21.15g  %21.15g \n", debugfail, rho0, u);
  }
#endif
...
#if(!OPTIMIZED)
	if( ltrace ) {
	  dualfprintf(fail_file,"enter utsq %21.15g %21.15g %21.15g %21.15g %21.15g\n",Bsq,QdotBsq,Qtsq,W4,Wsq) ;
	  fflush(fail_file);
	}
#endif
...
//
	...
	/*
	  if(utsq < 0. || utsq > UTSQ_TOO_BIG) {
	    dualfprintf(fail_file,"utsq failure %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g\n",utsq,Bsq,QdotBsq,Qtsq,W4,Wsq) ;
	    return(0.) ;
	  }
	*/
...
#if(!OPTIMIZED)
	if( ltrace ) {
	  dualfprintf(fail_file,"enter utsq  %21.15g %21.15g %21.15g %21.15g\n",Bsq,QdotBsq,Qtsq,Wsq) ;
	  fflush(fail_file);
	}
#endif
...
static void ncov_calc(FTYPE g00,FTYPE ncov[4]) 
{
	FTYPE lapse ;
			    ...
			    FTYPE (*res_func) (FTYPE []) )
{
  ...
  extern void my_lnsrch(int, FTYPE [], FTYPE, FTYPE [], FTYPE [], FTYPE [], FTYPE *, 
...
static void func_1d_orig(FTYPE x[], FTYPE dx[], FTYPE resid[], FTYPE jac[][NEWT_DIM], FTYPE *f, FTYPE *df, int n)
{
  ...
  gamma = ( utsq < 0. ) ? (1. ) : sqrt(1. + utsq) ;
  gamma_sq = gamma*gamma;

  rho0 = D/gamma ;

  w = W/gamma_sq;
  p_tmp = pressure_rho0_w_1dopt(rho0,w);
  ...
  *df = -2. * (*f);
  ...
  if( ltrace ) {
    ...
    dualfprintf(fail_file,"func_1d_orig(): dg = %21.15g,  dp = %21.15g \n", dg, dpdW);
    fflush(fail_file);
  }
#endif
...
static void bin_newt_data( FTYPE errx, int niters, int conv_type, int print_now  ) 
{
  ...
  const  long int n_output_freq = 128*128*2;
  FTYPE lerrx;
  ...
  if( print_now  ||  ( (n_bin_calls % n_output_freq) == 0 ) ) {
    ...
    fflush(fail_file);
    dualfprintf(fail_file,"t = %21.15g ,  n_beyond_range = %ld , n_bin_calls = %ld \n", t, n_beyond_range, n_bin_calls);

    /* ERRX */
    dualfprintf(fail_file,"ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--\n");
    dualfprintf(fail_file,"                         x");
    for( j = 0; j < N_CONV_TYPES; j++ ) { 
      dualfprintf(fail_file,"            N%d",j);
    }
    dualfprintf(fail_file,"\n");
    
    for( i = 0; i < NBINS; i++ ) { 
      dualfprintf(fail_file,"%21.15g ",xbin[i]);
      for( j = 0; j < N_CONV_TYPES; j++ ) { 
	dualfprintf(fail_file,"%13ld ", n_errx[j][i]);
      }
      dualfprintf(fail_file,"\n");
    }    
    dualfprintf(fail_file,"ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--ERRX-HISTOGRAM--\n");
    ...
    dualfprintf(fail_file,"NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--\n");
    dualfprintf(fail_file,"        niter");
    for( j = 0; j < N_CONV_TYPES; j++ ) { 
      dualfprintf(fail_file,"            N%d",j);
    }
    dualfprintf(fail_file,"\n");
    ...
    for( i = 0; i < N_NITER_BINS; i++ ) { 
      dualfprintf(fail_file,"%13d ", i);
      for( j = 0; j < N_CONV_TYPES; j++ ) { 
	dualfprintf(fail_file,"%13ld ", n_niters[j][i]);
      }
      dualfprintf(fail_file,"\n");
    }    
    dualfprintf(fail_file,"NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--NITER-HISTOGRAM--\n");
