...
int bound_prim_user(int boundstage, FTYPE prim[][N2M][N3M][NPR])
{
  ...
  if (mycpupos[1] == 0) {
	...
	LOOPBOUND1IN{
	  ...
	  if(steppart==3 &&  nstep==390 ){
	    dualfprintf(fail_file,"i=%d j=%d prim[B2]=%21.15g\n",i,j,prim[i][j][k][B2]);
	  }
	  ...
	  //pl=B2;
	  //	  prim[i][j][k][B2] = prim[ri][rj][rk][B2] * gdet[ri][rj][rk][CENT]/gdet[i][j][k][CENT] ;
	  //pl=B3;
	  //prim[i][j][k][pl] = prim[ri][rj][rk][pl] * gdet[ri][rj][rk][CENT]/gdet[i][j][k][CENT] ;

	  if(!isfinite(prim[i][j][k][B2])){
	    dualfprintf(fail_file,"i=%d j=%d steppart=%d nstep=%ld\n",i,j,steppart,nstep);
	    PLOOP(pl) dualfprintf(fail_file,"prim[%d]=%21.15g\n",pl,prim[i][j][k][pl]);
	    dualfprintf(fail_file,"extra: %d %d %d :: %21.15g %21.15g %21.15g %21.15g\n",i,j,k,prim[ri][rj][rk][pl] ,gdet[ri][rj][rk][CENT],gdet[i][j][k][CENT],prim[ri][rj][rk][pl] * gdet[ri][rj][rk][CENT]/gdet[i][j][k][CENT]) ;
	    myexit(2525);
	  }
