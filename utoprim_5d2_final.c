...
int Utoprim_5d2_final(FTYPE U[NPR], struct of_geom *ptrgeom,  int *lpflag,  FTYPE prim[NPR])
{
  ...
  if( ret ){
    *glpflag= UTOPRIMFAILCONV;
    ...
    ret_gam = gamma_calc(prim_tmp, ptrgeom, &gamma_tmp );
...
static void calc_errx_5d( FTYPE *W, FTYPE *vsq, FTYPE x[] ) 
{
  ...
  *vsq = 0. ;
  for(id=1;id<4;id++)
    for(jd=1;jd<4;jd++) 
      *vsq += ptrlgeom->gcov[id][jd]*x[UTCON1+id-1]*x[UTCON1+jd-1] ;      
...
static int dudp_calc_g(FTYPE *pr, struct of_state *q, struct of_geom *geom, FTYPE Am[][NEWT_DIM] )
{
  ...
  // get matrix
  dudp_calc(EVOLVENOENTROPY,pr,q,geom,alpha);
...
static int LU_decompose( FTYPE A[][NEWT_DIM], int permute[] )
{
  ...
  for( i = 0; i < n; i++ ) { 
    ...
    if( absmax == 0. ) { 
      dualfprintf(fail_file, "LU_decompose(): row-wise singular matrix: %d %d %d\n", icurr, jcurr,kcurr);
//      for(k=0;k<n;k++) { 
  ...
  */
  ...
  for( j = 0; j < n; j++ ) {
    ...
    */
    ...
    if( A[j][j] == 0. ) { 
      A[j][j] = absmin;
      //dualfprintf(fail_file, "LU_decompose(): column-wise singular matrix: %d %d %d\n", icurr, jcurr,kcurr);
      //return(1);
    }
