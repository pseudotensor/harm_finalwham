...
int eno_line_a2c( int whichquantity, int do_weight_or_recon, int whichreduce, int preforder, int pl, int bs, int ps, int pf, int bf, int *minorderit, int *maxorderit, int *shiftit, 
								  FTYPE *shockindicator, FTYPE (*df)[NBIGM], FTYPE (*dP)[NBIGM], FTYPE (*monoindicator)[NBIGM], 
									FTYPE *P, FTYPE *yin,  FTYPE *yout) 
...
//#endif
  ...
  res = eno_line_reconstruct( whichquantity, do_weight_or_recon, CVT_A2C, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator, df, dP, monoindicator, P, yin, yout, NULL ); 
...
int eno_line_c2a( int whichquantity, int do_weight_or_recon, int whichreduce, int preforder, int pl, int bs, int ps, int pf, int bf, int *minorderit, int *maxorderit, int *shiftit, 
								  FTYPE *shockindicator, FTYPE (*df)[NBIGM], FTYPE (*dP)[NBIGM], FTYPE (*monoindicator)[NBIGM], 
									FTYPE *P, FTYPE *yin,  FTYPE *yout) 
     //wrapper function that calls a more general conversion function, eno_line_reconstruct
{
  ...
  return( 0 );
	...
	res = eno_line_reconstruct( whichquantity, do_weight_or_recon, CVT_C2A, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator,  df, dP, monoindicator, P, yin, yout, NULL ); 
...
int eno_line_c2e( int whichquantity, int do_weight_or_recon, int whichreduce, int preforder, int pl, int bs, int ps, int pf, int bf, int *minorderit, int *maxorderit, int *shiftit, 
									FTYPE *shockindicator, FTYPE (*df)[NBIGM], FTYPE (*dP)[NBIGM], FTYPE (*monoindicator)[NBIGM], 
									FTYPE *P, FTYPE *yin, FTYPE *yout_left, FTYPE *yout_right ) 
{
#if( DO_CENT2EDGE == 0 )
  int i;
  for( i = ps; i <= pf; i++ ) {
  ...
  return 
		eno_line_reconstruct( whichquantity, do_weight_or_recon, CVT_C2E, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator,  df, dP, monoindicator, P, yin, yout_left, yout_right ); 
#endif
}
...
int eno_line_reconstruct(int whichquantity, int do_weight_or_recon, int cvt_type, int whichreduce, int preforder, int pl, 
												 ...
												 FTYPE *P, 
												 FTYPE *yin,  FTYPE *yout_left, FTYPE *yout_right) 
{
	int i;
	FTYPE simpleweight;
	int counter;
	...
	if( do_weight_or_recon == WEIGHT_CALC || do_weight_or_recon == ALL_CALC) { //peform the unoptimized weight calculation
		...
		for( i = ps; i <= pf; i++ ) {  
			...
			apply_additional_reduction_to_weights( cvt_type, whichreduce, maxorder, minorder, i, pl, bs, bf, shockindicator, 
				dP, monoindicator[MONOYIN], P, yin, stencil_weights_array );
			  ...
			  //SASMARKx: bad idea to use for the interpolation of conserved quantities because gamma is at a previous time step
				if( pl == NPR && cvt_type == CVT_C2E ) {  
					...
					//		stencil_weights_array[i].lower_order_fraction, nstep, steppart, iglobal, i, N2 - 1 - i );
					//}

					//do this only for gamma, which is the last, NPR-th, primitive; STORE_GAMMA_PRIM_REDUCTION_FRACTION == 1 means that gamma actually exists and is being interpolated
					//store the largest value of lower_order_fraction for the current point globally
					//weno_prim_lower_order_fraction[dirglobal][iglobal + i*di][jglobal + i*dj][kglobal + i*dk] = 
					//			MAX( stencil_weights_array[i].lower_order_fraction,
					//						weno_prim_lower_order_fraction[dirglobal][iglobal + i*di][jglobal + i*dj][kglobal + i*dk] );  

					//SUPERSASMARK: put in the check for cusp into the lower_order_fraction
					if( monoindicator[0][i] == 0 && (monoindicator[1][i] == 1 || monoindicator[2][i] == 1) ) {
					  if( fabs(yin[i]-yin[i-1]) + fabs(yin[i+1]-yin[i]) > 0.01 * yin[i] ) {
  						weno_prim_lower_order_fraction[dirglobal][iglobal + i*di][jglobal + i*dj][kglobal + i*dk] = 1.0;
					  }
					}
				}
					...
					if( whichquantity == ENOFLUX ) {
						...
						stencil_weights_array[i].lower_order_fraction = 
							MAX( stencil_weights_array[i].lower_order_fraction,
											weno_prim_lower_order_fraction[dirglobal][iglobal + i*di - (dirglobal==1)][jglobal + i*dj - (dirglobal==2)][kglobal + i*dk - (dirglobal==3)] );
					}
	...
	if( do_weight_or_recon == RECON_CALC || do_weight_or_recon == ALL_CALC ) {
		...
		for( i = ps; i <= pf; i++ ) {
				...
				num_orders = choose_weno_order( cvt_type, whichreduce, maxorder, minorder, i, pl, bs, bf, shockindicator, dP, monoindicator[MONOYIN], monoindicator[LEFTYOUT], P, yin, stencil_weights_array, &p_stencil_weights_array_to_be_used );
			#else
			...
			for( weno_output_count = 0; weno_output_count < weno_outputs[cvt_type].len; weno_output_count++ ) {
				...
				/////
				/////
				//New version: do not treat monoindicator here, only 5th order polynomial.  Initialize output values if they have not been assigned according to 5th point stencil in (J)SMONO
				if( monoindicator[MONOYIN][i] == 0.0 ) {
					...
					enodebugarray[iglobal + i*di][jglobal + i*dj][kglobal + i*dk][dirglobal-1][interporfluxglobal][pl][ENODEBUGPARAM_SMONO]++;
				}
#endif
				////////
				...
				////////
				if( monoindicator[MONOYIN][i] >= 0.0 && monoindicator[MONOYIN][i] < 1.0 ){
					...
					for( counter = 0; counter < num_orders; counter++ ) {  
						...
						if(  dirglobal<=2 && pl <=U2 && 
							...
							enodebugarray[iglobal + i*di][jglobal + i*dj][kglobal + i*dk][dirglobal-1][interporfluxglobal][pl][ENODEBUGPARAM_WENO3]++;
						}
						// WENO5:
							...
							enodebugarray[iglobal + i*di][jglobal + i*dj][kglobal + i*dk][dirglobal-1][interporfluxglobal][pl][ENODEBUGPARAM_WENO5]++;
						}
#endif
					}
					////////
											...
											weno_weights_t *stencil_weights_array ) 
{
	...
	dPi = dP[0][i0];
		...
		if(  dirglobal<=2 && pl <=U2 && lower_order_fraction > MAX(1.5 * lower_order_fraction_old, 0.01)  ) {
			enodebugarray[iglobal + i0*di][jglobal + i0*dj][kglobal + i0*dk][dirglobal-1][interporfluxglobal][pl][ENODEBUGPARAM_dPP]++;
		}
	#endif 
	...
	if( do_weno_lower_order_fraction ) {
		lower_order_fraction = MAX( lower_order_fraction, weno_lower_order_fraction[iglobal + i0*di][jglobal + i0*dj][kglobal + i0*dk][pl] );  //atch
		weno_lower_order_fraction[iglobal + i0*di][jglobal + i0*dj][kglobal + i0*dk][pl] = lower_order_fraction; //store the largest value of lower_order_fraction globally //atch
	}
#endif 
...
#if(DEBUGMONO||1)
  //	dualfprintf(fail_file,"t=%21.15g i=%d mi=%g\n",t,icurr,*monoindicator);
#endif
...
int compute_ac_ca_stencil_weights( int cvt_type, int pl, int order, int min_index, int max_index, FTYPE *monoindicator0, FTYPE *monoindicator1, FTYPE *uin, weno_weights_t *stencil_weights_out )
{
  ...
  FTYPE simpleweight;
  FTYPE sum_smoothness_indicators;
 ...
 // }
  ...
  sum_smoothness_indicators = normalize_array( order, DBL_MIN, smoothness_indicators );
...
int compute_optimized_ac_ca_stencil_weights( int cvt_type, int pl, int order, int min_index, int max_index, FTYPE *monoindicator, FTYPE *uin, weno_weights_t *stencil_weights_out )
{
  ...
  //normalize weights  SASMARK4 : DBL_MIN or add something else?
  normalize_array( order, DBL_MIN, stencil_pos_weights );
  normalize_array( order, DBL_MIN, stencil_neg_weights );
     ...
     //fills stencil_weights_out array with three sets of weight, of total length (3*order): roughness weights, leftface-optimized weights, rightface-optimized weights
{
  ...
  FTYPE sum_of_nonopt_stencil_weights;
     ...
     //fills stencil_weights_out array with three sets of weight, of total length (3*order): roughness weights, leftface-optimized weights, rightface-optimized weights
{
  ...
  for( shift = 0; shift <= order - 1; shift++ ) { 
	...
	//SASMARK4 : is DBL_MIN ok here?
	//normalize the optimized weights
  normalize_array( order, DBL_MIN, stencil_weights_leftface );
  normalize_array( order, DBL_MIN, stencil_weights_rightface );
...
#define LEFT_WEIGHT (order - 1)
	...
	int i, counter, il, ir;
	static weno_weights_t stencil_weights_array_static[NUM_POINTS];
	weno_weights_t *p_stencil_weights_array[NUM_POINTS];  //pointers to actual arrays with weights; used as an intermediary layer
	FTYPE lower_order_fraction;
	FTYPE dPi;
	#if( DOENODEBUG )
		FTYPE lower_order_fraction_old;  //atch debug
	...
	for( counter = 0; counter < NUM_POINTS; counter++ ) {
		//point the pointers to the arrays with precomputed weights
		//GODMARK: should not get memory violation
		p_stencil_weights_array[counter] = &stencil_weights_array[i0 + (counter - 1) * (max_order - 1)];  //initially assign the p_stencil_weights_array to point to precomputed weights
	}

#if( WENO_C2E_REDUCE_NEAR_CUSPS || WENO_AC_REDUCE_NEAR_CUSPS)
	if( (stencil_weights_array[i0].do_reduce & 1) != 0) {
		//set the right order
		stencil_weights_array_static[CENTER_POINT].do_reduce = 1; //indicates that the reduction to the 1st order was required; probably unnecessary, just set it to be something
		stencil_weights_array_static[CENTER_POINT].order = 1;
		stencil_weights_array_static[CENTER_POINT].len = 3;
		for( counter = 0; counter < stencil_weights_array_static[CENTER_POINT].len; counter++ ) {
			stencil_weights_array_static[CENTER_POINT].weights[counter] = 1.;  //set optimized, left and right weights to unity (because reducing to DONOR)
		}
		...
		p_stencil_weights_array[CENTER_POINT] = &stencil_weights_array_static[CENTER_POINT];

		*pp_stencil_weights_to_be_used = p_stencil_weights_array[CENTER_POINT];  //make sure that in all cases this is the actual pointer to where the weights are
	...
	if( whichreduce == WENO_REDUCE_TYPE_PPM ) {
		...
		//(p_stencil_weights_array[CENTER_POINT] may point to the same memory space as stencil_weights_array_static[1])
		stencil_weights_array_static[CENTER_POINT] = *(p_stencil_weights_array[CENTER_POINT]);

		//rescale the weights of current order for output; we have three sets of weights, each of order elements
		for( counter = 0; counter < stencil_weights_array_static[CENTER_POINT].len; counter++ ) {
			stencil_weights_array_static[CENTER_POINT].weights[counter] *= ( 1. - lower_order_fraction );
		}
		...
		compute_stencil_weights( cvt_type, pl, lower_order, bs - i0, bf - i0, &monoindicator0[i0], &monoindicator1[i0], &uin[i0], &stencil_weights_array_static[CENTER_POINT+1] );
		compute_optimized_stencil_weights( cvt_type, pl, lower_order, bs - i0, bf - i0, &monoindicator0[i0], &uin[i0], &stencil_weights_array_static[CENTER_POINT+1] );

		//rescale lower-order weights so that weights of current order and lower order now add up to unity
		for( counter = 0; counter < stencil_weights_array_static[CENTER_POINT+1].len; counter++ ) {
			stencil_weights_array_static[CENTER_POINT+1].weights[counter] *= lower_order_fraction;
		}
...
//		if( 1 == lower_order_fraction  
//#if( WENO_C2E_REDUCE_NEAR_CUSPS == 1 || WENO_AC_REDUCE_NEAR_CUSPS == 1)
//			|| stencil_weights_array[i0].do_reduce == 2 
//#endif
//			) {  //atch correct reduction
//				//the ratio is beyond transition region --> strong shock --> reduce stencil
//				lower_order = order - 1; 
//				for( counter = 0; counter < NUM_POINTS; counter++ ) { 
//					compute_stencil_weights( cvt_type, pl, lower_order, 
//						bs - i0 - (lower_order - 1)*(counter-1), 
//						bf - i0 - (lower_order - 1)*(counter-1),  
//						&monoindicator[i0 + (lower_order - 1)*(counter-1)],
//						&uin[i0 + (lower_order - 1)*(counter-1)], 
//						&stencil_weights_array_static[counter] ); 
//					p_stencil_weights_array[counter] = &stencil_weights_array_static[counter]; 
//				}
//				continue;
//		}
//		else if( w_ratio > MIN_TRANSITION_RATIO && w_ratio < MAX_TRANSITION_RATIO ) { 
		...
		if( 0.0 < lower_order_fraction ) {   //atch modified to always have two orders
			...
			//set the right order
			stencil_weights_array_static[CENTER_POINT].order = order;
...
void debugeno_compute(FTYPE p[N1M][N2M][N3M][NPR],FTYPE U[N1M][N2M][N3M][NPR],FTYPE debugvars[N1M][N2M][N3M][NUMENODEBUGS])
     ...
     // NPR*2 order*2*NPR NPR
{
  ...
  extern int get_shock_indicator(int dir, int interporflux, int bs, int ps, int pe, int be,  int i, int j, int k, int idel, int jdel, int kdel, FTYPE (*yin)[2][NBIGM], FTYPE *shockindicator);

  int a_minorderit[N1M];  //should actually be arrays but currently weno routines use only the 1st element of those arrays
  int a_maxorderit[N1M];
  int *minorderit;
  int *maxorderit;
  int preforder = 5;
  ...
  FTYPE a_yin[N1M]; 
  FTYPE a_yout_left[N1M], a_yout_right[N1M];
  FTYPE *yin = &a_yin[N1BND];
  FTYPE *yin1;
  FTYPE *yout_left = &a_yout_left[N1BND];
  FTYPE *yout_right = &a_yout_right[N1BND];
  int counter; 
  ...
  FTYPE a_shockindicator[N1M], *shockindicator = a_shockindicator + N1BND;

  assert( N2 != 1 || N3 != 1, "Debug output can only work in 1D so far; please make sure that N2 and N3 are equal to unity.\n" ) ;

  minorderit = a_minorderit + N1BND;
  maxorderit = a_maxorderit + N1BND;
  ...
  PLOOP(pl) {
    ...
    for( i = bs, j = 0, k = 0; i <= bf; i++ ) {
      ...
      debugvars[i][j][k][pl+start_prim] = p[i][j][k][pl];
    ...
    eno_line_c2e( whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, &shiftit, shockindicator, yin1, yout_left, yout_right ); 
		
    //copy p_l and p_r to the debug array for that pl
    for( i = ps, j = 0, k = 0; i <= pf; i++ ) {
      debugvars[i][j][k][start_index] = (FTYPE)i;
      debugvars[i][j][k][start_index+1] = (FTYPE)j;
      debugvars[i][j][k][start_index+2] = (FTYPE)k;

      //generate para4() left and right states
      para4( pl, &yin1[i], &left, &right, &dqleft, &dqcenter, &dqright );
      debugvars[i  ][j][k][pl+start_para_p_r] = left;
      debugvars[i+1][j][k][pl+start_para_p_l] = right;
			
      //atch modify indices in such a way that p_l and p_r sit on the same interface for the same i
      debugvars[i  ][j][k][pl+start_p_r] = yout_left[i];
      debugvars[i+1][j][k][pl+start_p_l] = yout_right[i];
	...
	if( counter < p_stencil_weights_array_to_be_used->len && p_stencil_weights_array_to_be_used->order == 3 ) {
	  debugvars[i][j][k][index] = p_stencil_weights_array_to_be_used->weights[counter];
	}
	else {
	  //if the weights do not exist, feed zeroes so that SM does not crash
	  debugvars[i][j][k][index] = 0.0;
	}
      }
    ...
    for( i = bs, j = 0, k = 0; i <= bf; i++ ) {
      //copy centered primitives to the debugvars array
      debugvars[i][j][k][start_Uavg+pl] = U[i][j][k][pl];
    ...
    for( i = ps, j = 0, k = 0; i <= pf; i++ ) {
      //atch modify indices in such a way that p_l and p_r sit on the same interface for the same i
      debugvars[i][j][k][start_Upoint+pl] = yout_left[i];
	...
	if( counter < p_stencil_weights_array_to_be_used->len && p_stencil_weights_array_to_be_used->order == 3 ) {
	  debugvars[i][j][k][start_weights_a2c + 2 * (order + order - 1) * pl + counter] = p_stencil_weights_array_to_be_used->weights[counter];
	}
	else {
	  //if the weights do not exist, feed zeroes so that SM does not crash
	  debugvars[i][j][k][start_weights_a2c + 2 * (order + order - 1) * pl + counter] = 0.0;
	}
      }
  ...
  get_shock_indicator(1, interporflux1, bs, ps, pf, bf,  i, j, k, idel, jdel, kdel, allpl_yin, shockindicator);

  index = start_shockindicator;
  for( i = ps, j = 0, k = 0; i <= pf; i++ ) {
    debugvars[i][j][k][index] = shockindicator[i];
  }
...
int check_symmetry_of_weno_output( int cvt_type, FTYPE (*monoindicator)[NBIGM], int pl, int ps, int pf, int maxorder, FTYPE *yout[MAX_NO_OF_WENO_OUTPUTS], weno_weights_t *stencil_weights_array )
{
  ...
  if( weno_outputs[cvt_type].len == 1 ) {
    ...
    for( i = 0; i <= pf-ps; i++ ) {
      for( j = 0; j < maxorder; j++ ) {
	if( stencil_weights_array[i+ps].weights[j] != stencil_weights_array[pf-i].weights[maxorder-1-j] || monoindicator[0][i+ps] != monoindicator[0][pf-i] ) {
	  dualfprintf( fail_file, "Asymmetry in the weights of c2e:  nstep = %ld, steppart = %d\n", nstep, steppart );
	  is_not_symmetric = 1;
	}
      }
      for( j = 0; j < 2 * maxorder; j++ ) {
	if( stencil_weights_array[i+ps].weights[j+maxorder] != stencil_weights_array[pf-i].weights[maxorder + 2*maxorder-1-j] && monoindicator[0][i+ps] != 1.0 ) {
