...
int mnewt(int whethertoreport,int ntrial, int mintrial, FTYPE x[], int n, int startp, FTYPE tolx, FTYPE tolf, FTYPE tolxallowed, FTYPE tolfallowed, FTYPE tolxreport, FTYPE tolfreport)
{
  ...
  if ((errf <= tolfallowed)&&(errx<=tolxallowed)) {
    ...
    if(debugfail>=2){
      ...
      // more flexible?
      coord(icurr,jcurr,kcurr,CENT,X);
      bl_coord(X,V);
      get_geometry(icurr,jcurr,kcurr,CENT,&geom);
      dualfprintf(fail_file,"i=%d j=%d k=%d \nx1=%21.15g x2=%21.15g x3=%21.15g \nV1=%21.15g V2=%21.15g V3=%21.15g \ng=%21.15g\n",startpos[1]+icurr,startpos[2]+jcurr,startpos[3]+kcurr,X[1],X[2],X[3],V[1],V[2],V[3],geom.g);
