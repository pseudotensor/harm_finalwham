...
int flux_compute_general(int i, int j, int k, int dir, struct of_geom *ptrgeom, FTYPE CUf, FTYPE *p_l, FTYPE *p_r, FTYPE *F, FTYPE *ctopptr)
{
  ...
  FTYPE F_l[NPR], F_r[NPR], U_l[NPR], U_r[NPR];
  int pl;
  ...
  //
  //////////////////////
  MYFUN(p2SFUevolve(dir, p_l, ptrgeom, &state_l, F_l, U_l),"step_ch.c:fluxcalc()", "p2SFUevolve()", 1);
  MYFUN(p2SFUevolve(dir, p_r, ptrgeom, &state_r, F_r, U_r),"step_ch.c:fluxcalc()", "p2SFUevolve()", 2);

  // usually "always" need cminmax_l cminmax_r and always need ctop
  get_wavespeeds(dir, ptrgeom, p_l, p_r, U_l, U_r, F_l, F_r, &state_l, &state_r, cminmax_l, cminmax_r, cminmax, ctopptr);

  //  if(nstep==0 && steppart==0 && ptrgeom->i==4 && ptrgeom->j==36){
  //    dualfprintf(fail_file,"dir=%d CUf=%21.15g ctop=%21.15g\n",dir,CUf,*ctopptr);
  //    PLOOP(pl) dualfprintf(fail_file,"p_l[%d]=%21.15g p_r[%d]=%21.15g\n",pl,p_l[pl],pl,p_r[pl]);
  //    PLOOP(pl) dualfprintf(fail_file,"U_l[%d]=%21.15g U_r[%d]=%21.15g\n",pl,U_l[pl],pl,U_r[pl]);
  //    PLOOP(pl) dualfprintf(fail_file,"F_l[%d]=%21.15g F_r[%d]=%21.15g\n",pl,F_l[pl],pl,F_r[pl]);
  //  }
  ...
  //  if(nstep==0 && steppart==0 && ptrgeom->i==4 && ptrgeom->j==36){
  //    PLOOP(pl) dualfprintf(fail_file,"F[%d]=%21.15g\n",pl,F[pl]);
  //  }
...
int p2SFUevolve(int dir, FTYPE *p, struct of_geom *ptrgeom, struct of_state *state, FTYPE *F, FTYPE *U)
{

  MYFUN(get_state(p, ptrgeom, state),"flux.c:p2SFUevolve", "get_state()", 1);
  MYFUN(primtoflux(UEVOLVE,p, state, dir, ptrgeom, F),"flux.c:p2SFUevolve()","primtoflux_calc() dir=1/2 l", 1);
  MYFUN(primtoflux(UEVOLVE,p, state, TT, ptrgeom, U),"flux.c:p2SFUevolve()", "primtoflux_calc() dir=l0", 1);
...
int flux_compute(int i, int j, int k, int dir, struct of_geom *geom, FTYPE *cminmax_l, FTYPE *cminmax_r, FTYPE *cminmax, FTYPE ctop, FTYPE CUf, FTYPE *p_l, FTYPE *p_r, FTYPE *U_l, FTYPE *U_r, FTYPE *F_l, FTYPE *F_r, FTYPE *F)
{
  ...
  if(fluxmethod==LAXFFLUX){
    PLOOP(pl) F[pl] = LAXFCOMPUTE(ctop,U_l[pl],U_r[pl],F_l[pl],F_r[pl]);
  }
    ...
    //
    /////////////////////////////////
    //    PLOOP(pl) choose_flux(i,j,k,pl,laxffrac,hllfrac);
...
#define MAXDPOP (0.3)
    ...
    if(cmax+cmin!=0.0){
      PLOOP(pl) F[pl] = HLLLAXF1(cmin,cmax,ctop,f_s, U_l[pl], U_r[pl], F_l[pl], F_r[pl]);
    }
    else PLOOP(pl) F[pl] = LAXFCOMPUTE(ctop,U_l[pl],U_r[pl],F_l[pl],F_r[pl]);
  
    
    
    //PLOOP(pl) dualfprintf(fail_file,"p_l[%d]=%g p_r[%d]=%g U_l[%d]=%g U_r[%d]=%g F_l[%d]=%g F_r[%d]=%g :: F[%d]=%g\n",pl,p_l[pl],pl,p_r[pl],pl,U_l[pl],pl,U_r[pl],pl,F_l[pl],pl,F_r[pl],pl,F[pl]);
...
int hllflux_compute(int dir,struct of_geom *geom, FTYPE cmin, FTYPE cmax, FTYPE ctop, FTYPE *p_l, FTYPE *p_r, FTYPE *U_l, FTYPE *U_r,FTYPE *F_l,FTYPE *F_r, FTYPE *F)
{
  ...
  if(cmax+cmin!=0.0){
    PLOOP(pl) F[pl] =  HLLCOMPUTE(cmin,cmax,U_l[pl],U_r[pl],F_l[pl],F_r[pl]);
  }
  else PLOOP(pl) F[pl] = LAXFCOMPUTE(ctop,U_l[pl],U_r[pl],F_l[pl],F_r[pl]);
...
int forceflux_compute(int dir,struct of_geom *geom, FTYPE cmin, FTYPE cmax, FTYPE ctop, FTYPE cforce, FTYPE *p_l, FTYPE *p_r, FTYPE *U_l, FTYPE *U_r,FTYPE *F_l,FTYPE *F_r, FTYPE *F)
{
  ...
  //
  ////////////////////////////
  PLOOP(pl) umid[pl] = UHALF(cforce,U_l[pl],U_r[pl],F_l[pl],F_r[pl]);

  //  PLOOP(pl) dualfprintf(fail_file,"%d : cforce=%21.15g : U_l[%d]=%21.15g U_r[%d]=%21.15g umid[%d]=%21.15g\n",geom->i,cforce,pl,U_l[pl],pl,U_r[pl],pl,umid[pl]);
  
  // set guess for inversion
  PLOOP(pl) pmid[pl]=0.5*(p_l[pl]+p_r[pl]);
  // get primitive pmid(umid)
  MYFUN(Utoprimgen(EVOLVEUTOPRIM,UEVOLVE, umid, geom, pmid),"flux.c:flux_compute()", "Utoprimgen", 1);
  doforceflux=1;
  if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
  ...
  //  PLOOP(pl) dualfprintf(fail_file,"%d : pmid[%d]=%21.15g p_l[%d]=%21.15g p_r[%d]=%21.15g\n",geom->i,pl,pmid[pl],pl,p_l[pl],pl,p_r[pl]);
  ...
  ////////////////////////////////////
  if(doforceflux){
    MYFUN(get_state(pmid, geom, &state),"flux.c:flux_compute()", "get_state()", 1);
    MYFUN(primtoflux(UEVOLVE, pmid, &state, dir, geom, fmid),"flux.c:flux_compute()","primtoflux_calc() dir=1/2 l", 1);
    
    // compute FORCE flux
    //    PLOOP(pl) F[pl] = FORCECOMPUTE(cforce,U_l[pl],U_r[pl],F_l[pl],fmid[pl],F_r[pl]);
    PLOOP(pl) F[pl] = GFORCECOMPUTE(MUSTACOEF,cforce,U_l[pl],U_r[pl],F_l[pl],fmid[pl],F_r[pl]);

    //    PLOOP(pl) dualfprintf(fail_file,"%d : F[%d]=%21.15g Fhll[%d]=%21.15g\n",geom->i,pl,F[pl],pl,HLLCOMPUTE(cmin,cmax,U_l[pl],U_r[pl],F_l[pl],F_r[pl]));
  }
  else{
    // reduce to HLL
    hllflux_compute(dir,geom,cmin,cmax,ctop,p_l,p_r,U_l,U_r,F_l,F_r,F);
...
int mustaflux_compute(int dir,struct of_geom *geom, FTYPE cmin_l, FTYPE cmin_r, FTYPE cmax_l, FTYPE cmax_r, FTYPE cmin, FTYPE cmax, FTYPE ctop, FTYPE cforce, FTYPE *p_l, FTYPE *p_r, FTYPE *U_l, FTYPE *U_r,FTYPE *F_l,FTYPE *F_r, FTYPE *F)
{
  ...
  PLOOP(pl) F[pl] = 0.5*(Fother[pl]+F[pl]);
#endif
  ...
  hllflux_compute(dir,geom,cminorig,cmaxorig,ctoporig,porig_l,porig_r,Uorig_l,Uorig_r,Forig_l,Forig_r,Fother);
  //  PLOOP(pl) if( (Fother[pl]-F[pl])/(fabs(Fother[pl])+SMALL)>0.9) F[pl]=Fother[pl];
  PLOOP(pl) if( fabs(Fother[pl]-F[pl])/(fabs(Fother[pl])+SMALL)>2.0) F[pl]=Fother[pl];
...
#if(0)
  hllflux_compute(dir,geom,cminorig,cmaxorig,ctoporig,porig_l,porig_r,Uorig_l,Uorig_r,Forig_l,Forig_r,Fother);
  PLOOP(pl) if( ((Fother[pl]<0.0)&&(F[pl]>0.0))||((Fother[pl]>0.0)&&(F[pl]<0.0)) ) F[pl]=Fother[pl];
#endif
...
#define MUSTACOEFTYPE 1
    ...
    MYFUN(Utoprimgen(EVOLVEUTOPRIM,UEVOLVE, U_l, geom, p_l),"flux.c:mustaflux_compute()", "Utoprimgen", 1);
    if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
    ...
    MYFUN(Utoprimgen(EVOLVEUTOPRIM,UEVOLVE, U_r, geom, p_r),"flux.c:mustaflux_compute()", "Utoprimgen", 1);
    if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
    ...
    // get fluxes
    MYFUN(get_state(p_l, geom, &state_l),"flux.c:flux_compute()", "get_state()", 1);
    MYFUN(primtoflux(UEVOLVE, p_l, &state_l, dir, geom, F_l),"flux.c:flux_compute()","primtoflux_calc() dir=1/2 l", 1);

    MYFUN(get_state(p_r, geom, &state_r),"flux.c:flux_compute()", "get_state()", 1);
    MYFUN(primtoflux(UEVOLVE, p_r, &state_r, dir, geom, F_r),"flux.c:flux_compute()","primtoflux_calc() dir=1/2 l", 1);
...
int musta2flux_compute(int dir,struct of_geom *geom, FTYPE cmin_l, FTYPE cmin_r, FTYPE cmax_l, FTYPE cmax_r, FTYPE cmin, FTYPE cmax, FTYPE ctop, FTYPE cforce, FTYPE *p_l, FTYPE *p_r, FTYPE *U_l, FTYPE *U_r,FTYPE *F_l,FTYPE *F_r, FTYPE *F)
{
  ...
  for(mustaloop=0;mustaloop<NUMMULTIMUSTAITERS;mustaloop++){
    ...
    */
...
#elif(WHICHFLUX==MUSTALAXF)
      cmusta=ctop;
      PLOOP(pl) Fnow[pl] = LAXFCOMPUTE(ctop,Unow_l[pl],Unow_r[pl],Fnow_l[pl],Fnow_r[pl]);
#endif
    ...
    for(mustacellloop=1;mustacellloop<=2*NUMLOCALCELLS;mustacellloop++){
      ...
      // Open Riemann fan
      PLOOP(pl) Unow[pl] -= MULTIMUSTACOEF*(Fnow_r[pl]-Fnow_l[pl])/cmusta;


      // get new primitive pnow from Unow
      MYFUN(Utoprimgen(EVOLVEUTOPRIM,UEVOLVE, Unow, geom, pnow),"flux.c:mustaflux_compute()", "Utoprimgen", 1);
      if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
	if(debugfail>=1) dualfprintf(fail_file,"Failed to find inversion for MUSTAFORCEFLUX(right): nstep=%ld t=%21.15g i=%d j=%d k=%d\n",nstep,t,geom->i,geom->j,geom->k);
	domustaflux=0;
	break;
      ...
      // get corrected centered fluxes
      MYFUN(get_state(pnow, geom, &state),"flux.c:flux_compute()", "get_state()", 1);
      MYFUN(primtoflux(UEVOLVE, pnow, &state, dir, geom, Fnow),"flux.c:flux_compute()","primtoflux_calc() dir=1/2 l", 1);
      // now have p, U, F at center again

      // get min and max wave speeds
      MYFUN(vchar(pnow, &state, dir, geom, cmaxnow, cminnow, &ignorecourant),"step_ch.c:fluxcalc()", "vchar() dir=1or2", 2);
  ...
  PLOOP(pl) F[pl]=Fmusta_edge[NUMLOCALCELLS+1][pl];
...
void choose_flux(int i, int j, int k, int pl, FTYPE *laxffrac,FTYPE *hllfrac)
{
  ...
  if(pl<B1){
    if(pflag[i][j][k][FLAGREALFLUX]==HLLFLUX){ hllfrac[pl]=1.0; laxffrac[pl]=0.0; }
