...
{ 
    ...
    for( i = -NBIGBND; i < NBIG + NBIGBND; i++ ){
			monoindicator[0][i] = 0.0;
			monoindicator[1][i] = 0.0;
			monoindicator[2][i] = 0.0;
		}
		return;
	}
	...
	if(recontype==CVT_A2C){
		for( i = bs+order-1; i <= be-order+1; i++ ) {  
			...
			// check for cusp
			if((check_for_cusp_new(&yin[i],&df[0][i],&df[1][i]))  ){
				set_as_rough(recontype, i, yin, yout, monoindicator);
			}
			else
#endif
			{
				...
				monoindicator[0][i] = indicator;
#else
				monoindicator[0][i] = (int)(indicator + 0.5);
#endif
				monoindicator[1][i] = monoindicator[0][i];
				if( indicator != 0 ) {
					a2c_simple_eno( 5, &yin[i], &yout[0][i] );
				}
			}
		} // end for loop over i
	}
...
#endif
				monoindicator[1][i] = monoindicator[0][i];
				monoindicator[2][i] = monoindicator[0][i];

				if( indicator != 0 ){
						//get the left interface value
						c2e_simple_eno( 5, 0, &yin[i], &yout[0][i] );

						//get the right interface value
						c2e_simple_eno( 5, 1, &yin[i], &yout[1][i] );
				}
			}// end else if smooth
		}
	}
	...
	//
		...
		for( i = bs+order-1; i <= be-order+1; i++ ) {  
...
#endif
			{
				...
				if( indicator != 0 ){
						c2a_simple_eno( 5, &yin[i], &yout[0][i] );
				}
			}// end else if smooth
		}//end for
	}
...
void set_as_rough(int recontype, int i, FTYPE *yin, FTYPE (*yout)[NBIGM], FTYPE (*monoindicator)[NBIGM])
{
	...
	//if((shockindicator[i]>=0.1)||(check_for_cusp_new(&df[0][i],&df[1][i],&yin[i]))  ){// does somewhat better than not checking
	//if(0){
...
{
#define MAX_DER 3
	FTYPE left_der[MAX_DER+1], right_der[MAX_DER+1], minmod_der[MAX_DER+1];
	...
	cutoff_value = norm * epsilon + DBL_MIN;
