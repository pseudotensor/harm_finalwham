		   ...
		   FILE ** fp, void *jonio, void *writebuf)
{
  ...
  if (myid == 0) {
    for (l = 0; l < numprocs; l++) {
      if(l!=0){
	MPI_Irecv(writebuf, N1 * N2 * N3 * numcolumns, datatype, l, l, MPI_COMM_WORLD, &rrequest);
		      ...
		      FILE ** fp,void *writebuf)
{
  ...
  if (stage == 1) {
	...
	DUMPGENLOOP for (col = 0; col < numcolumns; col++) {
	  ...
	  if(datatype==MPI_UNSIGNED_CHAR) fscanf(fp[(bufferwritemap)%numfiles],"%uc",&sendbuf1[bufferwritemap]);
	...
	if(l!=0){
      ...
      MPI_Irecv(writebuf, N1 * N2 * N3 * numcolumns, datatype, 0, myid, MPI_COMM_WORLD, &rrequest);
      MPI_Wait(&rrequest, &mpichstatus);	// writebuf used until
    }
  } else if (stage == 2) {
    free(writebuf);
...
void prminmaxsum(FTYPE p[][N2M][N3M][NPR], int start,int nmemb, FTYPE *maxptr, FTYPE*minptr,FTYPE*sumptr)
{
  ...
  for(pl=start;pl<start+nmemb;pl++){    
    if(domax){
      maxsend = maxptr[pl];
      MPI_Allreduce(&maxsend, &maxptr[pl], 1, MPI_FTYPE, MPI_MAX, MPI_COMM_WORLD);
    }
    if(domin){
      minsend = minptr[pl];
      MPI_Allreduce(&minsend, &minptr[pl], 1, MPI_FTYPE, MPI_MIN, MPI_COMM_WORLD);
    }
    if(dosum){
      sumsend = sumptr[pl];
      MPI_Allreduce(&sumsend, &sumptr[pl], 1, MPI_FTYPE, MPI_SUM, MPI_COMM_WORLD);
    }
  }
#endif
}
...
void myfread(int bintxt, MPI_Datatype datatype, void *ptr, int start, int nmemb, int i, int j, int k, FILE**stream,void*writebuf)
{
  ...
  if(mpicombine==0){
	...
	if (datatype == MPI_UNSIGNED_CHAR) fscanf(stream[streamnum],"%uc",&ptr1[pl]); // ok to get remark #181 in icc
