...
{ 
  ...
  if(bintxt==TEXTOUTPUT){// should always be TEXTOUTPUT
    if( (realtotalsize[1]>1)&&(realtotalsize[2]>1)&&(realtotalsize[3]>1)){
      // assume if 3D that no header wanted
      //      fprintf(headerptr, "RAW\n# t=%21.15g vartype=%2d  whichpl=%2d scale=%2d limits=%2d \n%i %i %i\n255\n", t, imagevartype, imagewhichpl, imagescale, imagelimits, realtotalsize[1],realtotalsize[2],realtotalsize[3]);
    }
    // otherwise assume want header if 2D only
    else if( (realtotalsize[2]>1)&&(realtotalsize[3]>1)){
      fprintf(headerptr, "RAW\n# t=%21.15g vartype=%2d  whichpl=%2d scale=%2d limits=%2d \n%i %i\n255\n", t, imagevartype, imagewhichpl, imagescale, imagelimits, realtotalsize[2],realtotalsize[3]);
    }
    else if( (realtotalsize[1]>1)&&(realtotalsize[3]>1)){
      fprintf(headerptr, "RAW\n# t=%21.15g vartype=%2d  whichpl=%2d scale=%2d limits=%2d \n%i %i\n255\n", t, imagevartype, imagewhichpl, imagescale, imagelimits, realtotalsize[1],realtotalsize[3]);
    }
    else if( (realtotalsize[1]>1)&&(realtotalsize[2]>1)){
      fprintf(headerptr, "RAW\n# t=%21.15g vartype=%2d  whichpl=%2d scale=%2d limits=%2d \n%i %i\n255\n", t, imagevartype, imagewhichpl, imagescale, imagelimits, realtotalsize[1],realtotalsize[2]);
    }
  }
  else{
