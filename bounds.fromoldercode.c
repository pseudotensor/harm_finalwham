...
int bound_prim_user(int boundstage, FTYPE prim[][N2M][N3M][NPR])
{
  ...
  if((boundstage==STAGE0)||(boundstage==STAGEM1)){
	...
	for (j = 0; j < N2; j++) {
	  ...
	  for(i=-1;i>=-N1BND;i--){
	    for(pl=RHO;pl<=UU;pl++){
	      prim[i][j][k][pl] = prim[ri][rj][k][pl] * gdet[ri][rj][k][CENT]/gdet[i][j][k][CENT] ;
	    }
	    // set  	    ucon[TT,etc.]
	    ...
	    vcon2pr(WHICHVEL,vcon,&geom,prim[i][j][k]); // get in terms of primitive velocity
	  ...
	  for(i=-1;i>=-N1BND;i--){
#if(WHICHVEL==VEL4)
	    get_geometry(i, j, k, CENT, &geom);
	    inflow_check_4vel(1,prim[i][j][k],&geom, Dt) ;
#elif(WHICHVEL==VEL3)
	    get_geometry(i, j, k, CENT, &geom);
	    inflow_check_3vel(1,prim[i][j][k],&geom, Dt) ;
	    ...
	    get_geometry(i,j,k,CENT,&geom) ;
	    inflow_check_rel4vel(1,prim[i][j][k],&geom,Dt) ;
	    if(limit_gamma(GAMMAMAX,prim[i][j][k],&geom,Dt)>=1)
	...
	for (j = 0; j < N2; j++) {
	  ...
	  for(i=N1;i<=N1-1+N1BND;i++){
	    ...
	    prim[i][j][k][pl] = prim[ri][rj][k][pl] * (1. - 2*(i-ri)*dx[1]) ;
	    for(pl=U2;pl<=U3;pl++){
	      prim[i][j][k][pl] = prim[ri][rj][k][pl] * (1. - (i-ri)*dx[1]) ;
	    }
	...
	ri=i;
	rj=0;
	for(j=-N2BND;j<=-1;j++) PLOOP(pl)  prim[i][j][k][pl] = prim[ri][rj+(rj-j-1)][k][pl];
      }
    }
	...
	ri=i;
	rj=N2-1;
	for(j=N2;j<=N2-1+N2BND;j++) PLOOP(pl)  prim[i][j][k][pl] = prim[ri][rj+(rj-j+1)][k][pl];
      }
    }
