...
int Utoprim_ldz(FTYPE *U, struct of_geom *ptrgeom, int *lpflag, FTYPE *pr)
{
  ...
  beta[0] = 0.;
  SLOOPA(j) beta[j] = ptrgeom->gcon[0][j] * alpha * alpha;
  ...
  */
  ...
  /* calculate Q^2 */
  Qsq = 0.;
  SLOOP(j,k) Qsq += Q[j] * Q[k] * ptrgeom->gcov[j][k];

  /* calc. S */
  Sc = 0.;
  SLOOP(j,k) Sc += ptrgeom->gcov[j][k] * Q[j] * B[k];

  /* calc. Bsq */
  Bsq = 0.;
  SLOOP(j,k) Bsq += ptrgeom->gcov[j][k] * B[j] * B[k];
  ...
  if(wvsq_solv_ldz(&vsq, &W)>=1){
    ...
    *glpflag = UTOPRIMFAILCONVUTSQ;
    ...
    //    if(debugfail>=1) PLOOP(k) dualfprintf(fail_file,"%d %d U[%d]=%21.15g pr[%d]=%21.15g\n",ptrgeom->i+startpos[1],ptrgeom->j+startpos[2],k,U[k],k,pr[k]);
    return(0);
  }
  if((vsq>=1.0)||(vsq<=0.0)){
  ...
  *glpflag= UTOPRIMNOFAIL;
  ...
  PLOOP(k){
    sprintf(testtext[k],"%21.15g",pr[k]);
	...
	//PLOOP(k) 	dualfprintf(fail_file,"ldz nan: pr[%d]=%21.15g U=%21.15g\n",k,pr[k],U[k]);
  ...
  PLOOP(k){
    sprintf(testtext[k],"%21.15g",pr[k]);
    if((!strcmp("nan",testtext[k]))||(!strcmp("inf",testtext[k]))||(!strcmp("-inf",testtext[k]))){
      dualfprintf(fail_file,"ldz nan/inf @ i=%d j=%d, k=%d\n",ptrgeom->i,ptrgeom->j,k); fflush(fail_file);
      *glpflag = UTOPRIMFAILCONVW;
      PLOOP(k) pr[k]=pr0[k];// revert
      return(0);
      // fail, but correct
    }
  }
  ...
  PLOOP(k){
    if(fabs(pr[k])>prMAX[k]){
      ...
      PLOOP(k) 	dualfprintf(fail_file,"%21.15g\n",pr0[k]);
      ...
      PLOOP(k) 	dualfprintf(fail_file,"%21.15g\n",pr[k]);
      PLOOP(k) 	dualfprintf(fail_file,"vsq=%21.15g gamma=%21.15g\n",vsq,gamma);
...
void funcorig(FTYPE *x,FTYPE *f,FTYPE *df)
{
  FTYPE vsq,glf1,cc,igam1,ee,wsq,dw ;
  FTYPE a2,a1,a0,q,r,th,w1,vp2,cosa ;
