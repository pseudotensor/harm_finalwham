...
int Utoprim_2d_final(FTYPE U[NPR], struct of_geom *ptrgeom,  int *lpflag,  FTYPE prim[NPR])
{
...
static int find_root_2D_gen(FTYPE x0, FTYPE *xnew) 
{
  ...
  if(icurr==0 && jcurr==31 && nstep==9 && steppart==2){
    dualfprintf(fail_file,"x[0]=%21.15g x[1]=%21.15g\n",x[0],x[1]);
  }
#endif
...
static void func_vsq(FTYPE x[], FTYPE dx[], FTYPE resid[], FTYPE jac[][NEWT_DIM], FTYPE *f, FTYPE *df, int n)
{
  ...
  dPdW   = dpdW_calc_vsq( W, D, vsq );
  dPdvsq = dpdvsq_calc( W, D, vsq );
...
#if(CRAZYDEBUG)
  if(icurr==0 && jcurr==31 && nstep==9 && steppart==2){
    dualfprintf(fail_file,"Qdotn=%21.15g QdotBsq=%21.15g Qtsq=%21.15g Bsq=%21.15g vsq=%21.15g W=%21.15g p_tmp=%21.15g -Qdotn-W=%21.15g\n",Qdotn,QdotBsq,Qtsq,Bsq,vsq,W,p_tmp,-Qdotn-W);
