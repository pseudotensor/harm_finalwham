...
void compute_jmonotonicity_line(int recontype, int whichreduce, int preforder, int pl, int bs, int ps, int pe, int be, int *minorder, int *maxorder, int *shift,   FTYPE *shockindicator, FTYPE (*df)[NBIGM], FTYPE (*monoindicator)[NBIGM], FTYPE *yin,  FTYPE (*yout)[NBIGM])
{
  int i;
  ...
  for(i=ps;i<=pe;i+=pointdi){ // loop over points of interest
    ...
    check_for_cusp_indicator(&yin[i], &df[0][i], &df[1][i], &cuspindicator);
    ...
    // check for cusp
    if((check_for_cusp(&yin[i],&df[0][i],&df[1][i]))  ){
    //dualfprintf(fail_file,"shocki[%d]=%21.15g\n",i,shockindicator[i]);
    //if((shockindicator[i]>=0.1)||(check_for_cusp(&df[0][i],&df[1][i],&yin[i]))  ){// does somewhat better than not checking
    //if(0){
...
#if(CHECKYIN)
      if((WHICHENOTYPE==0)||(WHICHENOTYPE==1)) check_mono_input(&yin[i], &df[0][i], &df[1][i], &dpdown, &dpup, &ddpdown, &ddpup);
      else check_mono_input_eno4(&yin[i], &df[0][i], &df[1][i], &dpdown, &dpup, &ddpdown, &ddpup);
	...
	monoindicator[2][i]=1;      
      ...
      if((USESHOCKINDICATOR||(USECUSPINDICATOR==2))&&(monoindicator[0][i]==1)){
	if(recontype==CVT_C2E){
	...
	monoindicator[2][i]=1;      
      ...
      yout[0][i]=yin[i];
      monoindicator[0][i]=0;// unknown roughness
      monoindicator[1][i]=1;// set
    }
#endif
  ...
  for(i=ps;i<=pe;i++){ // loop over points of interest
    if(monoindicator[0][i]!=monoindicator[0][(pe+ps) - i]){
      dualfprintf(fail_file,"problem at i=%d io=%d : %d %d :: recontype=%d\n",i,pe+ps-i, monoindicator[0][i],monoindicator[0][pe-i],recontype);
      for(i=ps;i<=pe;i++){
	dualfprintf(fail_file,"mon[%d]=%d :: %21.15g %21.15g :: %21.15g :: %21.15g %21.15g\n",i,monoindicator[0][i],yin[i],yout[0][i],df[1][i],df[0][i],df[0][i+1]);
      }
      myexit(1);
    }
  }
...
void c2e_simple_limiter(int WHICHLIMITERTOREDUCETO, FTYPE *yin, FTYPE *valueleft, FTYPE *valueright)
{
  ...
  if(WHICHLIMITERTOREDUCETO<PARA){
    ...
    *valueright=yin[0]+0.5*mydq;
    ...
    para4(RHO,yin,valueleft,valueright);
  }
...
void compute_check_output(int i, int bs, int be, int recontype, int preforder, FTYPE (*df)[NBIGM], FTYPE *yin, FTYPE (*monoindicator)[NBIGM], FTYPE (*yout)[NBIGM])
{
  ...
  if(recontype==CVT_C2E){
    ...
    if(WHICHENOTYPE==0){
      ...
      check_mono_c2e_output(i, preforder, &df[0][i], &df[1][i], pleft, pright, &yin[i], &monoindicator[0][i], &monoindicator[1][i], &monoindicator[2][i],&yout[0][i],&yout[1][i]);
	...
	monoindicator[0][i+1]=0;
      ...
      // check if edge values are monotonic
      check_mono_c2e_output_eno4(i, preforder, &df[0][i], &df[1][i], pleft, &yin[i], monoindicator,yout);
    }
    ...
    check_mono_a2c_c2a_output(preforder, &df[0][i], &df[1][i], &pcenter, &yin[i], &monoindicator[0][i], &monoindicator[1][i],&yout[0][i]);
  }
}
